...
[![CI](https://github.com/steelpipe75/padtools_ts/actions/workflows/pipeline.yml/badge.svg)](https://github.com/steelpipe75/padtools_ts/actions/workflows/pipeline.yml) [![codecov](https://codecov.io/gh/steelpipe75/padtools_ts/branch/main/graph/badge.svg)](https://codecov.io/gh/steelpipe75/padtools_ts) [![smithery badge](https://smithery.ai/badge/steelpipe75/padtools_ts)](https://smithery.ai/servers/steelpipe75/padtools_ts)
...
- Swagger UI: [https://padtools-ts.steelpipe75.workers.dev/docs](https://padtools-ts.steelpipe75.workers.dev/docs)
...
#### AIエージェントへの設定例 (Gemini CLI)

以下のコマンドを実行するか、設定ファイルを直接編集することで、Gemini CLI からツールとして利用可能になります。

**コマンドラインで追加する場合:**

```bash
gemini mcp add --cwd "C:/path/to/padtools_ts" padtools npx -y tsx C:/path/to/padtools_ts/src/mcp/server.ts
```

**設定ファイル (`~/.gemini/settings.json`) を直接編集する場合:**
...
- Smithery: [https://smithery.ai/servers/steelpipe75/padtools_ts](https://smithery.ai/servers/steelpipe75/padtools_ts)

**Gemini CLI での設定例:**

コマンドラインで追加する場合:
```bash
gemini mcp add --transport http padtools https://padtools-ts.steelpipe75.workers.dev/mcp
```
...
```json
{
  "mcpServers": {
    "padtools": {
      "httpUrl": "https://padtools-ts.steelpipe75.workers.dev/mcp"
    }
  }
}
```
...
3.  ブラウザで表示された画面にて以下の設定を行います：
    ...
    - **URL**: 上記の URL (`https://padtools-ts.steelpipe75.workers.dev/mcp` または `http://localhost:3000/mcp`) を入力
...
-   [hono-openapi](https://github.com/rhinobase/hono-openapi): OpenAPI仕様の生成に使用。 (MIT License)
-   [@hono/standard-validator](https://github.com/honojs/middleware/tree/main/packages/standard-validator): バリデーション仕様の定義に使用。 (MIT License)
...
- 公開APIサーバー (Cloudflare Workers) : [https://padtools-ts.steelpipe75.workers.dev/docs](https://padtools-ts.steelpipe75.workers.dev/docs)
- Smithery (MCP Server) : [https://smithery.ai/servers/steelpipe75/padtools_ts](https://smithery.ai/servers/steelpipe75/padtools_ts)
