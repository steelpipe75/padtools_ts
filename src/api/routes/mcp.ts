...
export default function createServer(
  _context: ServerContext<z.infer<typeof configSchema>>,
) {
  if (!mcpServer) {
    ...
    mcpServer.registerPrompt(
      ...
      {
        description:
          "SPD（Simple PAD Description）表記法を例付きで説明します。",
      },
      async () => {
    ...
    mcpServer.registerPrompt(
      ...
      async (args) => {
        const text = await handleGenerateSpdPrompt(
