...
const outputSetting = [
  { file: "./web/svg/render_options/commands_diagram_Original.svg", option: "orignal" },
  { file: "./web/svg/render_options/commands_diagram_TerminalOffset.svg", option: "TerminalOffset" }
]
...
(async () => {
  for (const setiing of outputSetting) {
    ...
    exec(command, (error, stdout, stderr) => {
      if (error) {
        console.error(`Error converting ${input} to ${outputSvg}: ${error.message}`);
        return;
      }
      if (stderr) {
