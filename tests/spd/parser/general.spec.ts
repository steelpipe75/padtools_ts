...
Process 2`;
  ...
  it("callコマンドをパースすること", () => {
    ...
    expect(ast).toEqual({
      type: "nodeList",
      children: [{ type: "call", text: "MyFunction", childNode: null }],
    } as NodeListNode);
  });
...
:terminal End`;
  ...
  it("callコマンドをパースすること", () => {
    ...
    expect(ast).toEqual({
      type: "nodeList",
      children: [{ type: "call", text: "SubRoutine", childNode: null }],
    } as NodeListNode);
  });
  ...
  it("commentコマンドをパースすること", () => {
    ...
    expect(ast).toEqual({
      type: "nodeList",
      children: [{ type: "comment", text: "This is a comment" }],
    } as NodeListNode);
  });
...
:terminal End`;
    ...
    expectedCasesA.set("caseA1", {
      type: "process",
      text: "Process C",
      childNode: null,
    });
    expectedCasesA.set("caseA2", {
      type: "process",
      text: "Process D",
      childNode: null,
    });
    const expectedCasesB = new Map<string, Node | null>();
    expectedCasesB.set("caseB1", {
      type: "process",
      text: "Process G",
      childNode: null,
    });
    expectedCasesB.set("caseB2", {
      type: "process",
      text: "Process H",
      childNode: null,
    });
...
Process 2
`;
    const errorHandler = jest.fn((_line, _lineNo, err) => {
      return err instanceof UnknownCommandException;
    });

    const ast = parse(spd, errorHandler);
