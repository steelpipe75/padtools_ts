...
Process A`;
  ...
  it(":elseが:ifの後にない場合にUnexpectedElseExceptionをスローすること(Processの後)", () => {
    const spd = `Process
:else`;
    expect(() => SPDParser.parse(spd)).toThrow(UnexpectedElseException);
  });
...
:else some_arg
\tFalse Branch`;
    expect(() => SPDParser.parse(spd)).toThrow(NotRequireArgumentException);
  });
...
\tAnother False Branch`;
    expect(() => SPDParser.parse(spd)).toThrow(new UnexpectedElseException());
  });
});
