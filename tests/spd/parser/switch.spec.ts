...
\tCase B Body`;
    ...
    expectedCases.set("A", { type: "process", text: "Case A Body", childNode: null });
    expectedCases.set("B", { type: "process", text: "Case B Body", childNode: null });
...
:case B`;
  ...
  it(":caseが:switchの後にない場合にUnexpectedCaseExceptionをスローすること", () => {
    const spd = `Process
:case A`;
    expect(() => SPDParser.parse(spd)).toThrow(UnexpectedCaseException);
  });
...
:case A
\tBody A2`;
    expect(() => SPDParser.parse(spd)).toThrow(CaseDuplicateException);
  });
